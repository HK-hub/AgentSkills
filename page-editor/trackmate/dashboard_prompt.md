...
|---------|----------------|---------|
| session_start | Skill调用 / `skill_invoked` | 会话ID / `session_id`；调用来源 / `source`；编辑模式 / `editor_mode`（file/url） |
| on_event | 任务完成 / `task_completed` | 会话ID / `session_id`；完成状态 / `status`（值域：success/fail/cancel）；修改元素数量 / `elements_modified`；修改类型列表 / `change_types`（style,text,attribute,delete） |
| on_error | 异常发生 / `error_occurred` | 错误类型 / `error_type`（值域：runtime/timeout/dependency/user_cancel）；错误摘要 / `error_message`；发生阶段 / `phase`；错误码 / `error_code`（可选） |
| session_end | 会话结束 / `session_end` | 会话ID / `session_id`；会话时长秒 / `duration_seconds`；结束原因 / `reason`（user_close/timeout/error） |

### 2. 业务私有事件

本 Skill 的业务指标已全部复用通用事件的参数，无额外业务私有事件。
...
```
┌─────────────────────┬─────────────────────┬─────────────────────┐
│ 📈 平均修改元素数     │ 📈 今日保存次数       │ 📈 使用模式分布       │
│                     │                     │                     │
│ AVG(elements)       │ task_completed 总数  │ file vs url 占比     │
├─────────────────────┴─────────────────────┴─────────────────────┤
│                                                                 │
│  📊 修改类型分布（最近7天饼图）                                     │
│  style / text / attribute / delete 各占比                        │
│                                                                 │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│  📉 每日平均修改元素数趋势（最近14天折线图）                          │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```
...
| 图卡 | 计算逻辑 (伪SQL) |
|------|-----------------|
| 平均修改元素数 | `SELECT AVG(CAST(elements_modified AS BIGINT)) FROM events WHERE event_code = 'task_completed' AND ds >= CONCAT(DATE_SUB(TODAY(), 7), '00') AND ds <= CONCAT(TODAY(), '23')` |
| 今日保存次数 | `SELECT COUNT(*) FROM events WHERE event_code = 'task_completed' AND ds >= CONCAT(TODAY(), '00') AND ds <= CONCAT(TODAY(), '23')` |
| 使用模式分布 | `SELECT editor_mode, COUNT(*) AS cnt FROM events WHERE event_code = 'skill_invoked' AND ds >= CONCAT(DATE_SUB(TODAY(), 7), '00') AND ds <= CONCAT(TODAY(), '23') GROUP BY editor_mode` |
| 修改类型分布 | `SELECT change_types, COUNT(*) AS cnt FROM events WHERE event_code = 'task_completed' AND ds >= CONCAT(DATE_SUB(TODAY(), 7), '00') AND ds <= CONCAT(TODAY(), '23') GROUP BY change_types ORDER BY cnt DESC` |
| 每日平均修改元素数趋势 | `SELECT SUBSTR(ds, 1, 8) AS dt, AVG(CAST(elements_modified AS BIGINT)) AS avg_elements FROM events WHERE event_code = 'task_completed' AND ds >= CONCAT(DATE_SUB(TODAY(), 14), '00') AND ds <= CONCAT(TODAY(), '23') GROUP BY SUBSTR(ds, 1, 8) ORDER BY dt` |
